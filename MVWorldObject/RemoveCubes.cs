...
public static class RemoveCubes
{
	...
	public static void HandleRemoveCubes(List<CommonOverlapArg> overlapArgs, float radius, Vector3 worldPos, float centerDamage, DamageFallOffType damageFallOffType, MVWorldObject fineGrainedTerrainWorldObject)
	{
		//IL_0017: Unknown result type (might be due to invalid IL or missing references)
		//IL_00bf: Unknown result type (might be due to invalid IL or missing references)
		foreach (CommonOverlapArg overlapArg2 in overlapArgs)
		{
			if (!SphereOverlapTest.OverlapWo(overlapArg2, radius, worldPos, out var overlapResult))
			{
				continue;
			}
			if (overlapArg2.wo.WorldObjectType == WorldObjectType.CubeModelPrototypeTerrain)
			{
				ICubeModel cubeModel = (ICubeModel)overlapArg2.wo;
				foreach (OverlapCubeData cube in overlapResult.cubes)
				{
					if (CubeIsDestroyed(cubeModel, cube.cubePos, centerDamage))
					{
						if (cube.overlapState == OverlapState.OnEdge)
						{
							MoveCube(cubeModel, (ICubeModel)fineGrainedTerrainWorldObject, cube.cubePos);
						}
						cubeModel.RemoveCubeNetworkUpdate(cube.cubePos);
					}
				}
				CommonOverlapArg overlapArg = new CommonOverlapArg(fineGrainedTerrainWorldObject);
				if (!SphereOverlapTest.OverlapWo(overlapArg, radius, worldPos, out overlapResult))
				{
					continue;
				}
				foreach (OverlapCubeData cube2 in overlapResult.cubes)
				{
					((ICubeModel)fineGrainedTerrainWorldObject).RemoveCubeNetworkUpdate(cube2.cubePos);
				}
				continue;
			}
			foreach (OverlapCubeData cube3 in overlapResult.cubes)
			{
				ICubeModel cubeModel2 = (ICubeModel)overlapArg2.wo;
				if (CubeIsDestroyed(cubeModel2, cube3.cubePos, centerDamage))
				{
					cubeModel2.RemoveCubeNetworkUpdate(cube3.cubePos);
				}
			}
		}
	}

	private static bool CubeIsDestroyed(ICubeModel cm, IntVector pos, float damage)
	{
		if (MaterialPhysicalProperties.GetPhysicalProperty(cm.GetCubeBase(pos).FaceMaterials[0]).toughness < damage)
		{
			return true;
		}
		return false;
	...
	private static void MoveCube(ICubeModel from, ICubeModel to, IntVector fromPos)
	{
