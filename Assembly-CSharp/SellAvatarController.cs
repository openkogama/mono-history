...
public class SellAvatarController : MonoBehaviour
{
	...
	private void ScreenShotCallback(Texture2D texture, string successMessage)
	{
		...
		MVGameControllerBase.Game.AddAvatarToAvatarShopInventory(woID, 0, nameField.text, texture.EncodeToPNG());
	}
	...
	private void OnAddToMarketplace(bool added)
	{
		...
		if (added)
		{
			text = ((!metaData.isOnMarketPlace) ? "Avatar is now on the marketplace." : "Avatar updated on marketplace.");
		}
		else
		{
			text = ((!metaData.isOnMarketPlace) ? "Failed to put Avatar on the marketplace." : "Failed to update Avatar.");
		}
		ExecuteEvents.ExecuteHierarchy(gameObject, null, (IModalPopupCreator x, BaseEventData y) =>
		{
	...
	public void OnRemovePressed()
	{
		...
		MVGameControllerBase.Game.DeleteAvatarFromShopInventory(woID);
	}
