...
public class CFX2_Demo : MonoBehaviour
{
	...
	private void OnMouseDown()
	{
		...
		if (GetComponent<Collider>().Raycast(Camera.main.ScreenPointToRay(MVInputWrapper.GetPointerPosition()), out hitInfo, 9999f))
		{
	...
	private IEnumerator RandomSpawnsCoroutine()
	{
		while (true)
		{
			...
			if (orderedSpawns)
			{
				particles.transform.position = transform.position + new Vector3(order, particles.transform.position.y, 0f);
				order -= step;
				if (order < 0f - range)
				{
					order = range;
			...
			{
				particles.transform.position = transform.position + new Vector3(Random.Range(0f - range, range), 0f, Random.Range(0f - range, range)) + new Vector3(0f, particles.transform.position.y, 0f);
			}
	...
	private void Update()
	{
		if (MVInputWrapper.DebugGetKeyDown(KeyCode.LeftArrow))
		{
			prevParticle();
		}
		else if (MVInputWrapper.DebugGetKeyDown(KeyCode.RightArrow))
		{
			nextParticle();
		}
	}
