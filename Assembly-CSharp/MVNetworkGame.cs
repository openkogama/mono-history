...
public class MVNetworkGame : IPhotonPeerListener
{
	private class EventHandling
	{
		...
		private void HandleEvent(MVEventCodes eventCode, EventData photonEvent)
		{
			...
			case MVEventCodes.Join:
			{
				...
				MVPlayer mVPlayer2 = new MVPlayer(num6, profileID3, userName, regionCode, buildTarget, isReady: false);
				mVPlayer2.Team = team2;
				networkGame.MVPlayerContainer.Add(mVPlayer2);
				break;
			}
			...
			case MVEventCodes.CreateGameSnapshot:
			{
				...
				MVGameStateType gameStateType = (MVGameStateType)(int)photonEvent[64];
				int startTime = (int)photonEvent[66];
				int duration = (int)photonEvent[65];
				...
				int num4 = (int)photonEvent[34];
				if (num4 % 1000 != 0)
				{
					Debug.LogError("stepTimestamp is not correctly incremented");
				}
				networkGame.logicObjectManager = new LogicObjectManagerClient(num4, trackLoops: false);
				networkGame.logicObjectManagerClientWrapper = new LogicObjectManagerClientWrapper(networkGame, num4);
				break;
			}
			...
			case MVEventCodes.RequestFriends:
				networkGame.OnRequestFriendsResponse((Dictionary<object, object>)photonEvent[50]);
				break;
			...
			case MVEventCodes.GetActiveAvatar:
				networkGame.OnGetActiveAvatarResponse((int)photonEvent[21]);
				break;
			case MVEventCodes.Leave:
			{
				int num5 = (int)photonEvent[254];
				if (num5 != networkGame.LocalPlayer.ActorNr)
				{
					if (networkGame.MVPlayerContainer.ContainsKey(num5))
					{
						MVPlayer mVPlayer = networkGame.MVPlayerContainer[num5];
						Dictionary<object, object> dictionary6 = new Dictionary<object, object>();
						dictionary6[(byte)0] = num5;
						dictionary6[(byte)3] = mVPlayer.Username;
						dictionary6[(byte)6] = MVGameControllerBase.Game.Friends.IsFriend(mVPlayer.ProfileID);
						MVGameControllerBase.PostGameMsg(MVGameMsgType.UserLeft, dictionary6);
						networkGame.gameStatCounterManager.RemoveStatsFromActor(num5);
					}
					networkGame.MVPlayerContainer.Remove(num5);
				}
				else
				{
			...
			case MVEventCodes.UnregisterWorldObject:
				networkGame.OnUnregisterWorldObjectEvent((int)photonEvent[21]);
				break;
			...
			case MVEventCodes.UnregisterPrototype:
				networkGame.OnUnregisterPrototypeEvent((int)photonEvent[46]);
				break;
			case MVEventCodes.UpdatePrototype:
				networkGame.worldNetwork.WorldInventory.OnUpdatePrototypeEvent((int)photonEvent[46], (byte[])photonEvent[48]);
				break;
			case MVEventCodes.UpdatePrototypeScale:
				break;
			case MVEventCodes.UpdateWorldObjectData:
				networkGame.worldNetwork.WorldObjectClientManagerNetwork.OnUpdateWorldObjectDataEvent((int)photonEvent[21], (Dictionary<object, object>)photonEvent[17]);
				break;
			case MVEventCodes.UpdateWorldObjectDataPartial:
			{
				int worldObjectID9 = (int)photonEvent[21];
				Dictionary<object, object> worldObjectData = (Dictionary<object, object>)photonEvent[17];
			...
			{
				int worldObjectID8 = (int)photonEvent[21];
				Dictionary<object, object> worldObjectDataToRemove = (Dictionary<object, object>)photonEvent[18];
			...
			case MVEventCodes.UpdateWorldObjectRunTimeData:
				...
				{
					networkGame.worldNetwork.WorldObjectClientManagerNetwork.OnUpdateWorldObjectRunTimeDataEvent((int)photonEvent[21], (Dictionary<object, object>)photonEvent[69]);
				}
				break;
			case MVEventCodes.AddLink:
				networkGame.OnAddLinkEvent((int)photonEvent[56], (int)photonEvent[55], (int)photonEvent[57]);
				break;
			case MVEventCodes.RemoveLink:
				networkGame.OnRemoveLinkEvent((int)photonEvent[57]);
				break;
			case MVEventCodes.AddObjectLink:
				networkGame.OnAddObjectLinkEvent((int)photonEvent[56], (int)photonEvent[55], (int)photonEvent[57]);
				break;
			case MVEventCodes.RemoveObjectLink:
				networkGame.OnRemoveObjectLinkEvent((int)photonEvent[57]);
				break;
			case MVEventCodes.RemoveItemFromInventory:
				networkGame.OnRemoveItemFromInventory((int)photonEvent[39]);
				break;
			case MVEventCodes.FriendRequest:
			{
				int friendID2 = (int)photonEvent[51];
				int profileID2 = (int)photonEvent[11];
				int friendProfileID = (int)photonEvent[52];
				networkGame.OnFriendRequestEvent(friendID2, profileID2, friendProfileID);
				break;
			}
			case MVEventCodes.FriendUpdate:
			{
				int friendID = (int)photonEvent[51];
				int profileID = (int)photonEvent[11];
				FriendStatus status = (FriendStatus)(int)photonEvent[53];
				networkGame.OnFriendUpdateEvent(friendID, profileID, status);
				break;
			}
			case MVEventCodes.TriggerBoxEnter:
			{
				int worldObjectID7 = (int)photonEvent[21];
				int actorNr2 = (int)photonEvent[254];
				networkGame.OnTriggerBoxEnterEvent(actorNr2, worldObjectID7);
				break;
			}
			case MVEventCodes.TriggerBoxExit:
			{
				int worldObjectID6 = (int)photonEvent[21];
				int actorNr = (int)photonEvent[254];
				networkGame.OnTriggerBoxExitEvent(actorNr, worldObjectID6);
				break;
			}
			case MVEventCodes.TriggerBoxStayBegin:
			{
				int worldObjectID5 = (int)photonEvent[21];
				int instigatorId = (int)photonEvent[254];
				networkGame.OnTriggerBoxStayBegin(worldObjectID5, instigatorId);
				break;
			}
			case MVEventCodes.TriggerBoxStayEnd:
			{
				int worldObjectID4 = (int)photonEvent[21];
			...
			case MVEventCodes.WoUniquePrototype:
				networkGame.OnWoUniquePrototypeEvent((int)photonEvent[21], (int)photonEvent[46]);
				break;
			case MVEventCodes.GameStateChange:
				networkGame.networkGameStateListener.ChangeState((MVGameStateType)(int)photonEvent[64], (int)photonEvent[66], (int)photonEvent[65], fromGameSnapshot: false);
				break;
			case MVEventCodes.PropertiesChanged:
			{
				...
				Debug.Log(networkGame.MVPlayerContainer.GetPlayerUnsafe(num3).Username);
				{
			...
			case MVEventCodes.ResetLogicChunk:
				networkGame.OnResetLogicChunkEvent((int)photonEvent[21]);
				break;
			case MVEventCodes.PickupItemStateChange:
				networkGame.OnPickupItemStateChangeEvent((PickupItemState)(int)photonEvent[70], (int)photonEvent[21], (int)photonEvent[254]);
				break;
			case MVEventCodes.UpdateLineOfFire:
			{
				...
				networkGame.OnUpdateLineOfFire((int)photonEvent[21], camOrigin, camDir);
				break;
			}
			...
			case MVEventCodes.SetWorldObjectsToPurchasedEvent:
				networkGame.OnSetWorldObjectsToPurchasedEvent((int)photonEvent[11], (int)photonEvent[39]);
				break;
			...
			{
				Dictionary<object, object> dictionary4 = (Dictionary<object, object>)photonEvent[71];
				int seatOwnerWoID = (int)dictionary4[(byte)4];
				int worldObjectID3 = (int)dictionary4[(byte)0];
				networkGame.PlayerController.OnAttachWorldObjectToSeat((int)photonEvent[254], seatOwnerWoID, worldObjectID3, (byte)photonEvent[141]);
				break;
			}
			case MVEventCodes.DetachWorldObjectFromVehicle:
			{
				int id2 = (int)photonEvent[21];
			...
			case MVEventCodes.SpawnVehicleWithDriver:
			{
				...
				int cloneLinkId = (int)photonEvent[57];
				int cloneObjectLinkId = (int)photonEvent[91];
				int takeTime = (int)photonEvent[34];
				...
				networkGame.PlayerController.OnAttachWorldObjectToSeat((int)photonEvent[254], num2, worldObjectID2, (byte)photonEvent[141]);
			...
			case MVEventCodes.StartRewardCountDown:
			{
				...
				int countDownStartTick = (int)photonEvent[34];
			...
			case MVEventCodes.SetSayChatBubbleVisible:
			{
				...
				int worldObjectID = (int)photonEvent[21];
				networkGame.OnSetSayChatBubbleVisible(worldObjectID, visible);
				break;
			}
			...
			case MVEventCodes.LogicFastForward:
				...
				networkGame.logicObjectManagerClientWrapper.FastForward((int)photonEvent[34]);
				break;
			case MVEventCodes.LogicFastForwardEventImmediate:
				networkGame.logicObjectManagerClientWrapper.FastForwardImmediately((int)photonEvent[34]);
				break;
			...
			case MVEventCodes.XPReward:
				networkGame.LocalPlayer.AddXp((int)photonEvent[215], (XPRewardType)(byte)photonEvent[214], (int)photonEvent[84]);
				break;
			...
			{
				Quaternion quaternion2 = QuaternionCompression.ToQuaternion((byte[])photonEvent[157]);
				networkGame.OnUpdateHeadRotation((int)photonEvent[21], quaternion2.eulerAngles.y, quaternion2.eulerAngles.x);
				break;
			}
			case MVEventCodes.UpdatePointingAndHeadRotation:
			{
				Quaternion quaternion = QuaternionCompression.ToQuaternion((byte[])photonEvent[157]);
				float y = quaternion.eulerAngles.y;
				float x = quaternion.eulerAngles.x;
				networkGame.OnUpdateHeadRotation((int)photonEvent[21], y, x);
				networkGame.OnUpdatePointing((int)photonEvent[21], y, x);
				break;
			}
			case MVEventCodes.StartHeadShake:
				networkGame.OnStartHeadShake((int)photonEvent[21]);
				break;
			case MVEventCodes.StartHeadNod:
				networkGame.OnStartHeadNod((int)photonEvent[21]);
				break;
			case MVEventCodes.StartWave:
				networkGame.OnStartWave((int)photonEvent[21]);
				break;
	...
	private class LogicEventQueue
	{
		...
		private void HandleEvent(EventData photonEvent)
		{
			...
			{
			case MVEventCodes.LogicObjectFiringStateChange:
				((IIsLogicObjectFiringEventHandler)MVGameControllerBase.WOCM.GetWorldObjectClient((int)photonEvent[21])).OnIsFiringChanged((bool)photonEvent[204]);
				break;
			case MVEventCodes.CollectTheItemDropOff:
			{
				int[] array = (int[])photonEvent[71];
			...
			case MVEventCodes.GodzillaEnter:
			{
				int[] array2 = (int[])photonEvent[71];
				if (MVGameControllerBase.WOCM.GetWorldObjectClient(array2[0]) is GodzillaTrigger godzillaTrigger2)
				{
					godzillaTrigger2.OccupationChange(array2[1]);
				}
				break;
			}
			case MVEventCodes.GodzillaExit:
				if (MVGameControllerBase.WOCM.GetWorldObjectClient((int)photonEvent[21]) is GodzillaTrigger godzillaTrigger)
				{
	...
	public class OperationRequests
	{
		...
		private bool PublishPlanet(bool newImagePending)
		{
			...
			{
				Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
				dictionary.Add(63, newImagePending);
		...
		public void JoinGame()
		{
			...
			dictionary.Add(212, MVGameControllerBase.KoGaMaSettings.VersionString);
			List<FileData> cRCData = DllProtector.GetCRCData();
			dictionary.Add(213, JsonConvert.SerializeObject(cRCData));
		...
		public bool AddLink(Link link)
		{
			...
			{
				Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
				dictionary.Add(56, link.outputWOID);
				dictionary.Add(55, link.inputWOID);
		...
		public void UpdateWorldObject(int id, Vector3 position, byte[] rotation, TransformPackageType packageType)
		{
			...
			{
				Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
				dictionary.Add(21, id);
				dictionary.Add(34, networkGame.ServerTimeInMilliSeconds);
				TransformHelper.SetPosition(position, dictionary);
				dictionary.Add(157, rotation);
				dictionary.Add(35, (byte)packageType);
		...
		{
			Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
			dictionary.Add(21, worldObjectIDPickupOwner);
		...
		public void TransferOwnership(int worldObjectID, int ownerActorNr, Transform t)
		{
			...
			dictionary.Add(19, ownerActorNr);
			if (t == null)
			{
		...
		{
			Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
			dictionary.Add(86, (int)chatMsgType);
			dictionary.Add(87, gameMsgData);
		...
		public void UpdateWorldObjectData(int worldObjectID, Dictionary<object, object> worldObjectData)
		{
			Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
			dictionary.Add(21, worldObjectID);
			dictionary.Add(17, worldObjectData);
		...
		public void UpdateWorldObjectDataPartial(int worldObjectID, string keyPath, object value)
		{
			...
			dictionary3.Add(21, worldObjectID);
			dictionary3.Add(17, dictionary);
		...
		public void RemoveWorldObjectDataPartial(int worldObjectID, string keyPath)
		{
			...
			dictionary4.Add(21, worldObjectID);
			dictionary4.Add(18, dictionary);
		...
		public void RemoveWorldObjectDataPartial(int worldObjectID, Dictionary<object, object> woDataToRemove)
		{
			Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
			dictionary.Add(21, worldObjectID);
			dictionary.Add(18, woDataToRemove);
		...
		{
			Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
			dictionary.Add(16, type);
			dictionary.Add(22, groupId);
			dictionary.Add(17, woData);
			dictionary.Add(19, localOwner ? networkGame.LocalPlayer.ActorNr : 0);
		...
		public void UpdatePrototype(int worldInventoryID, byte[] prototypeData)
		{
			Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
			dictionary.Add(46, worldInventoryID);
			dictionary.Add(48, prototypeData);
		...
		{
			Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
			dictionary.Add(45, itemIdToSlotIndexTable);
		...
		{
			peer.OpCustom(94, new Dictionary<byte, object>(), sendReliable: true);
		}
		...
		public void TriggerBoxEnter(int triggerBoxOwnerId, int triggerInstigatorId)
		{
			...
			{
				Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
				dictionary.Add(21, new int[2] { triggerBoxOwnerId, triggerInstigatorId });
		...
		public void SetSayChatBubbleVisible(bool shouldShow)
		{
			...
			peer.OpCustom(93, dictionary2, sendReliable: true);
		}
	...
	private class OperationResponseHandling
	{
		...
		private void ExecuteOperationResponse(MVOperationCodes opCode, Dictionary<byte, object> returnValues, short returnCode)
		{
			...
			case MVOperationCodes.UnregisterWorldObject:
				...
				{
					networkGame.OnUnregisterWorldObjectResponse((int)returnValues[21]);
				}
				break;
			...
			case MVOperationCodes.AddWorldObjectToInventoryDev:
				networkGame.OnAddItemToInventory(returnValues, returnCode);
				networkGame.OnAddWorldObjectToInventoryResponseDev(returnCode, (int)returnValues[21], (int)returnValues[39]);
				break;
			...
			{
				int rootId = (int)returnValues[21];
			...
			case MVOperationCodes.AddItemToMarketPlace:
				if (returnCode == 0)
				{
					int itemID = (int)returnValues[39];
			...
			case MVOperationCodes.SetFirstTimeEvent:
				FirstTimeEventManager.OnFirstTimeEventResponse((FirstTimeEvent)(int)returnValues[191], (XPRewardType)(byte)returnValues[214]);
				break;
			default:
	...
	public MVNetworkGame()
	{
		...
		StreamingAssetInventory = new StreamingAssetInventory();
	}
	...
	private void OnJoinResponse(Dictionary<byte, object> returnValues)
	{
		...
		if (MVGameControllerBase.IsTouristSession)
		{
			mVLocalPlayer = new MVLocalPlayerTourist(actorNumber, MVGameControllerBase.GameSessionData.profileID, (string)returnValues[9], MVGameControllerBase.GameSessionData.language, num);
		}
		else
		{
	...
	{
		int id = (int)returnValues[21];
		int ownerActorNr = (int)returnValues[19];
		if (returnCode == 0)
		{
	...
	{
		int id = (int)returnValues[21];
		bool lockObject = (bool)returnValues[62];
		if (returnCode == 0)
		{
		}
	...
	private void OnUpdateWorldObjectEvent(EventData photonEvent)
	{
		if (MVGameControllerBase.JoinState == MVJoinState.Playing)
		{
			...
			networkTransformPackage.timestamp = (int)photonEvent[34];
			networkTransformPackage.packageType = (TransformPackageType)(byte)photonEvent[35];
	...
	private void OnTransferOwnershipEvent(EventData photonEvent)
	{
		...
		int num = (int)photonEvent[21];
		int ownerActorNr = (int)photonEvent[19];
	...
	public MVWorldObjectClient OnCloneWorldObjectTree(EventData eventData)
	{
		...
		int ownerActorNumber = (int)eventData[19];
		int cloneLinkId = (int)eventData[57];
	...
	private void OnSwitchAvatar(EventData eventData)
	{
		...
		if (playerContainer.LocalPlayer.ActorNr == num3)
		{
	...
	private void OnSetSayChatBubbleVisible(int worldObjectID, bool visible)
	{
		MVWorldObjectClient worldObjectClient = WorldObjectClientManager.GetWorldObjectClient(worldObjectID);
		if (worldObjectClient == null)
		{
