...
public class MVNetworkGame : IPhotonPeerListener
{
	...
	public bool PublishPlanet(ref string errorText)
	{
		...
		if (LocalPlayer.Level < PublishLevel)
		{
			errorText = TM._("You can not publish game before reaching level: " + PublishLevel);
			return false;
		}
		if (!isPublished)
		{
	...
	private void HandleOperationResponse(OperationResponse operationResponse)
	{
		...
		case MVOperationCodes.PublishPlanet:
		{
			...
			if (OnPublishedPlanet != null)
			{
				OnPublishedPlanet(empty);
			}
		...
		case MVOperationCodes.AddWorldObjectToInventory:
			if (OnAddWorldObjectToInventoryResponse != null)
			{
		...
		case MVOperationCodes.AddLink:
			if (returnCode != 0)
			{
				...
				worldNetwork.HandleAddLinkResponse(success: false, (int)parameters[56]);
			}
			else
			{
				worldNetwork.HandleAddLinkResponse(success: true, (int)parameters[56]);
			}
			break;
		case MVOperationCodes.RemoveLink:
			if (returnCode != 0)
			{
				...
				worldNetwork.HandleRemoveLinkResponse(success: false);
			}
			else
			{
				Debug.Log("RemoveLink SUCCESS");
				worldNetwork.HandleRemoveLinkResponse(success: true);
			}
			break;
		case MVOperationCodes.AddObjectLink:
			if (returnCode != 0)
			{
				...
				worldNetwork.HandleAddObjectLinkResponse(success: false, (int)parameters[56]);
			}
			else
			{
				worldNetwork.HandleAddObjectLinkResponse(success: true, (int)parameters[56]);
			}
			break;
		case MVOperationCodes.RemoveObjectLink:
			if (returnCode != 0)
			{
				...
				worldNetwork.HandleRemoveObjectLinkResponse(success: false);
			}
			else
			{
				worldNetwork.HandleRemoveObjectLinkResponse(success: true);
			}
			break;
		case MVOperationCodes.PurchaseItem:
			if (OnPurchaseItemResponse != null)
			{
				OnPurchaseItemResponse(returnCode);
			}
			break;
		case MVOperationCodes.TransferWorldObjectsToGroup:
			worldNetwork.WorldObjectClientManagerNetwork.HandleTransferWorldObjectsToGroup(returnCode == 0);
			break;
		...
		case MVOperationCodes.CloneWorldObjectTree:
		{
			...
			worldNetwork.WorldObjectClientManagerNetwork.OnCloneWorldObjectTreeResponse(returnCode == 0, num);
			break;
		}
	...
	public void OnEvent(EventData photonEvent)
	{
		...
		case 254:
		{
			...
			if (num8 != LocalPlayer.ActorNr)
			{
				MVPlayer mVPlayer2 = Players[num8];
				Debug.Log("Removed actor " + num8);
				Players.Remove(num8);
				gameStatCounterManager.RemoveStatsFromActor(num8);
				if (onPlayerListChanged != null)
				{
					onPlayerListChanged();
				}
		...
		case 4:
		{
			...
			this.worldNetwork.WorldObjectClientManagerNetwork.OnUpdateWorldObjectDataPartialEvent(worldObjectID9, worldObjectData);
			break;
		}
		case 5:
		{
			...
			this.worldNetwork.WorldObjectClientManagerNetwork.OnRemoveWorldObjectDataPartialEvent(worldObjectID8, worldObjectDataToRemove);
			break;
		}
		case 31:
		...
		case 15:
			...
			OnAddItemToInventoryEvent(photonEvent);
			break;
		case 16:
		...
		case 253:
		{
			...
			Debug.Log(Players[num7].Username);
			{
		...
		case 40:
			...
			break;
		case 41:
			OnRemoveTeamEvent((MVTeam)(int)Enum.ToObject(typeof(MVTeam), (int)photonEvent[89]), (Dictionary<object, object>)photonEvent[245]);
			break;
		case 42:
		...
		case 53:
		{
			...
			int num5 = (int)dictionary[(byte)3];
			...
			this.worldNetwork.OnCloneWorldObjectTreeEvent(ownerActorNumber, 0, cloneToRootGroup: true, spawnWorldObjectID, num5, cloneLinkId, cloneObjectLinkId);
			MVWorldObjectClient worldObjectClient = WorldObjectClientManager.GetWorldObjectClient(num5);
			MVWorldObjectClient.CallBackDelegate callBack = (MVWorldObjectClient wo) =>
			{
				wo.InteractionFlags = InteractionFlags.None;
			};
			worldObjectClient.TraverseRecursiveTail(callBack);
			PlayerController.OnAttachWorldObjectToSeat((int)photonEvent[254], num5, worldObjectID, (byte)photonEvent[142]);
