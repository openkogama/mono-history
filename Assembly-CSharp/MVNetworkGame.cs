...
public class MVNetworkGame : IPhotonPeerListener
{
	private class EventHandling
	{
		...
		private void HandleEvent(MVEventCodes eventCode, EventData photonEvent)
		{
			...
			{
				Dictionary<object, object> dictionary4 = (Dictionary<object, object>)photonEvent[70];
				int seatOwnerWoID = (int)dictionary4[(byte)4];
				int worldObjectID3 = (int)dictionary4[(byte)0];
				networkGame.PlayerController.OnAttachWorldObjectToSeat((int)photonEvent[254], seatOwnerWoID, worldObjectID3, (byte)photonEvent[140]);
				break;
			}
			...
			case MVEventCodes.SpawnVehicleWithDriver:
			{
				...
				networkGame.PlayerController.OnAttachWorldObjectToSeat((int)photonEvent[254], num2, worldObjectID2, (byte)photonEvent[140]);
			...
			case MVEventCodes.SetSayChatBubbleVisible:
			{
				...
				int worldObjectID = (int)photonEvent[20];
	...
	public class OperationRequests
	{
		...
		{
			Dictionary<byte, object> dictionary = new Dictionary<byte, object>();
			dictionary.Add(85, (int)chatMsgType);
			dictionary.Add(86, gameMsgData);
	...
	public void OnUpdateHeadRotation(int worldObjectID, float yaw, float pitch)
	{
		...
		{
			((AvatarLimbManagerRemote)((MVAvatar)worldObjectClient).LimbManager).UpdateHeadRotationRemotely(yaw, pitch);
		}
		else
		{
	...
	public void OnUpdatePointing(int worldObjectID, float yaw, float pitch)
	{
		...
		{
			((AvatarLimbManagerRemote)((MVAvatar)worldObjectClient).LimbManager).UpdatePointingRemotely(yaw, pitch);
		}
		else
		{
	...
	public void OnStartHeadShake(int worldObjectID)
	{
		...
		{
			((AvatarLimbManagerRemote)((MVAvatar)worldObjectClient).LimbManager).StartEmote(EmoteTypes.Shake);
		}
		else
		{
	...
	public void OnStartHeadNod(int worldObjectID)
	{
		...
		{
			((AvatarLimbManagerRemote)((MVAvatar)worldObjectClient).LimbManager).StartEmote(EmoteTypes.Nod);
		}
		else
		{
	...
	public void OnStartWave(int worldObjectID)
	{
		...
		{
			((AvatarLimbManagerRemote)((MVAvatar)worldObjectClient).LimbManager).StartEmote(EmoteTypes.wave);
		}
		else
		{
