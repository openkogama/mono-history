...
public class MobileAdManagerTestScript : MonoBehaviour
{
	...
	public void OnGUI()
	{
		...
		{
			mobileAdManager.RequestRewardedAd(RewardedAdCallback, AdContext.None);
		}
		Rect position4 = new Rect(x2, 0.225f * (float)Screen.height, width, height);
		if (GUI.Button(position4, "Request\nInterstitial"))
		{
			mobileAdManager.RequestInterstitial(InterstitialCallback, AdContext.None);
		}
		Rect position5 = new Rect(x2, 0.4f * (float)Screen.height, width, 0.5f * (float)Screen.height);
		...
		GUI.Label(position5, text, gUIStyle);
		Rect position6 = new Rect(x, 0.4f * (float)Screen.height, width, 0.5f * (float)Screen.height);
		GUIStyle gUIStyle2 = new GUIStyle();
		gUIStyle2.wordWrap = true;
		gUIStyle2.fontStyle = FontStyle.Bold;
		gUIStyle2.fontSize = 20;
		GUI.Label(position6, mobileAdManager.ToString(), gUIStyle2);
	}
