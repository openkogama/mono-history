...
public class ThemeSelection : MonoBehaviour
{
	...
	{
		if (themeRepo.CurrentlySerializedTheme != null)
		{
		...
		foreach (ThemeData themeData in data)
		{
			...
			if (themeRepo.CurrentlySerializedTheme != null && themeRepo.CurrentlySerializedTheme.Identifier == themeData.themeIdentifier)
			{
	...
	{
		PreviewTheme = themeRepo.CreatePreviewTheme(identifier);
		menuController.OpenSettingsForPreview(PreviewTheme, data);
	}

	private void ShowThemeRemovalWarning()
	{
		string msg = string.Format("{0}\n{1}", TM._("This will remove the current theme. The theme will be permanently lost.)"), TM._("Are you sure you want to do this?"));
		ExecuteEvents.ExecuteHierarchy(gameObject, null, (IModalPopupCreator x, BaseEventData y) =>
		{
			x.Create(msg, OnThemeRemovalWarningResolved, TM._("Theme purchase"));
		});
	}
