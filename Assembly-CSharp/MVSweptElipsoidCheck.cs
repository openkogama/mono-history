...
public static class MVSweptElipsoidCheck
{
	...
	private static Vector3[] cachedCornersWorldSpace = new Vector3[8];

	private static Vector3[] cachedFaceWorldSpace = new Vector3[4];
	...
	private static bool LayerScan(ref VoxelHit vh, Vector3 radius, float maxRadius, GameObject chunk, float distance, MVCubeModelBase wo, PhysicsCollisionData collisionData, Ray ray, Ray[] boundRays)
	{
		...
		collisionState.localToElipsoidSpace = worldToElipsoidSpace * wo.GameObject.transform.localToWorldMatrix;
		...
		Vector3 planeNormal = GetPlaneNormal(collisionState.localDirection, collisionData.transform);
	...
	public static Vector3 GetPlaneNormal(Vector3 localDir, Transform t)
	{
		//IL_017a: Unknown result type (might be due to invalid IL or missing references)
	...
	private static Vector3 GetMaxAngleLocalAxisVector(Ray ray, MVCubeModelBase cmb)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			val = cmb.GameObject.transform.rotation * array[i];
