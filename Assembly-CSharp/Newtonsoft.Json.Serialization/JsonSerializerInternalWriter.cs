...
internal class JsonSerializerInternalWriter : JsonSerializerInternalBase
{
	...
	private void SerializeValue(JsonWriter writer, object value, JsonContract valueContract, JsonProperty member, JsonContract collectionValueContract)
	{
		...
		else if ((jsonConverter != null || (jsonConverter = valueContract.Converter) != null || (jsonConverter = Serializer.GetMatchingConverter(valueContract.UnderlyingType)) != null || (jsonConverter = valueContract.InternalConverter) != null) && jsonConverter.CanWrite)
		{
		...
		{
			((JToken)value).WriteTo(writer, (Serializer.Converters == null) ? null : Serializer.Converters.ToArray());
		}
	...
	private bool ShouldWriteReference(object value, JsonProperty property, JsonContract contract)
	{
		...
		if (!flag.HasValue)
		{
			flag = ((!(contract is JsonArrayContract)) ? new bool?(HasFlag(Serializer.PreserveReferencesHandling, PreserveReferencesHandling.Objects)) : new bool?(HasFlag(Serializer.PreserveReferencesHandling, PreserveReferencesHandling.Arrays)));
		}
		if (!flag.Value)
		{
			return false;
	...
	private bool CheckForCircularReference(object value, ReferenceLoopHandling? referenceLoopHandling, JsonContract contract)
	{
		...
		{
			return ((value is Vector2 || value is Vector3 || value is Vector4 || value is Color || value is Color32) ? ReferenceLoopHandling.Ignore : referenceLoopHandling.GetValueOrDefault(Serializer.ReferenceLoopHandling)) switch
			{
				...
				_ => throw new InvalidOperationException("Unexpected ReferenceLoopHandling value: '{0}'".FormatWith(CultureInfo.InvariantCulture, Serializer.ReferenceLoopHandling)), 
			};
		}
		return true;
	}
	...
	private void SerializeObject(JsonWriter writer, object value, JsonObjectContract contract, JsonProperty member, JsonContract collectionValueContract)
	{
		...
		if ((!isReference.HasValue) ? HasFlag(Serializer.PreserveReferencesHandling, PreserveReferencesHandling.Objects) : isReference.Value)
		{
	...
	private void SerializeList(JsonWriter writer, IWrappedCollection values, JsonArrayContract contract, JsonProperty member, JsonContract collectionValueContract)
	{
		...
		bool? isReference = contract.IsReference;
		bool flag = ((!isReference.HasValue) ? HasFlag(Serializer.PreserveReferencesHandling, PreserveReferencesHandling.Arrays) : isReference.Value);
		...
		JsonContract collectionValueContract2 = Serializer.ContractResolver.ResolveContract(contract.CollectionItemType ?? typeof(object));
	...
	private string GetReference(JsonWriter writer, object value)
	{
		try
		{
			return Serializer.ReferenceResolver.GetReference(this, value);
		}
	...
	private bool ShouldWriteType(TypeNameHandling typeNameHandlingFlag, JsonContract contract, JsonProperty member, JsonContract collectionValueContract)
	{
		...
		if (HasFlag((!typeNameHandling.HasValue) ? Serializer.TypeNameHandling : typeNameHandling.Value, typeNameHandlingFlag))
		{
			return true;
		}
		if (member != null)
		{
			...
			if (((!typeNameHandling2.HasValue) ? Serializer.TypeNameHandling : typeNameHandling2.Value) == TypeNameHandling.Auto && contract.UnderlyingType != member.PropertyType)
			{
	...
	private void SerializeDictionary(JsonWriter writer, IWrappedDictionary values, JsonDictionaryContract contract, JsonProperty member, JsonContract collectionValueContract)
	{
		...
		JsonContract collectionValueContract2 = Serializer.ContractResolver.ResolveContract(contract.DictionaryValueType ?? typeof(object));
		int top = writer.Top;
